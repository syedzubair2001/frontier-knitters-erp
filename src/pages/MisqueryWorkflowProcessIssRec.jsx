...
export default function MisqueryWorkflowProcessIssRec() {
  ...
  const [activeStyleGroup, setActiveStyleGroup] = useState(false);
  const [styleGroup, setStyleGroup] = useState('-- All --');
  ...
  const handleClear = () => {
    ...
    setActiveSeason(false); setSeason('-- All --');
    setActiveStyle(false); setStyle('-- All --');
    setActiveStyleGroup(false); setStyleGroup('-- All --');
    setActiveBrand(false); setBrand('-- All --');
    setItem('-- All --'); setColor('-- All --'); setSize('-- All --');
    setOrderNo(''); setRefNo('');
  ...
  const radioDropdownFields = [
    ...
    { label: 'Buyer',        active: activeBuyer,        setActive: setActiveBuyer,        value: buyer,        setValue: setBuyer,        list: BUYER_LIST },
    ...
    { label: 'Merchandiser', active: activeMerchandiser, setActive: setActiveMerchandiser, value: merchandiser, setValue: setMerchandiser, list: ['-- All --'].concat(MERCHANDISER_LIST.slice(1)) },
    { label: 'Season',       active: activeSeason,       setActive: setActiveSeason,       value: season,       setValue: setSeason,       list: SEASON_LIST },
    { label: 'Style',        active: activeStyle,        setActive: setActiveStyle,        value: style,        setValue: setStyle,        list: ['-- All --'].concat(STYLE_LIST.slice(1)) },
    { label: 'Style Group',  active: activeStyleGroup,   setActive: setActiveStyleGroup,   value: styleGroup,   setValue: setStyleGroup,   list: STYLE_GROUP_LIST },
    { label: 'Brand',        active: activeBrand,        setActive: setActiveBrand,        value: brand,        setValue: setBrand,        list: ['-- All --'].concat(BRAND_LIST.slice(1)) },
  ];
          ...
          <div>
            ...
            <div style={{ backgroundColor: '#ffffff', borderRadius: 8, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#0f172a' }}>Process Issue/Receipt Summary — Results</h3>
                ...
                <table style={{ width: '100%', minWidth: 1000, borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f1f5f9', borderBottom: '1px solid #cbd5e1' }}>
                      ...
                      <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700 }}>Issue Qty</th>
                      <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700 }}>Receipt Qty</th>
                      <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700 }}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td colSpan={10} style={{ padding: 32, textAlign: 'center', color: '#64748b', fontStyle: 'italic' }}>
                        Process Issue/Receipt Summary data will be displayed here.
                      </td>
                    </tr>
                  </tbody>
                </table>
