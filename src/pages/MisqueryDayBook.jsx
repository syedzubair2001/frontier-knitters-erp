...
export default function MisqueryDayBook() {
  ...
  const [displayStatus, setDisplayStatus] = useState('Both'); // 'Inward', 'Outward', 'Both'
  ...
  const handleClear = () => {
    ...
    setCompany('--Select--'); setProcessType('--Select--'); setItemGroup('--Select--');
    setStore('--Select--'); setItem('--Select--'); setColor('--Select--'); setSize('--Select--');
    setOrderNo(''); setRefNo('');
    setFromDate(''); setToDate('');
    setStatus('All'); setDisplayStatus('Both');
    setDateWiseTotal(false); setOpeningStock(false);
    setExportData(false);
    setSubmitted(false);
  };
        ...
        {!submitted && (
          <form onSubmit={handleSubmit} style={{ backgroundColor: '#ffffff', borderRadius: 8, padding: '20px 24px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
              ...
              <div className="field">
                <label>Ref No (Wildcard % allowed)</label>
            ...
            {/* Row 4: Radios and Checkboxes */}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, marginBottom: 20 }}>
              
              <div style={{ padding: 16, backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, flex: 1, minWidth: 300 }}>
              ...
              <div style={{ padding: 16, backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, flex: 1, minWidth: 200 }}>
              ...
              <div style={{ padding: 16, backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, flex: 1, minWidth: 250 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: '#334155', cursor: 'pointer' }}>
                    <input type="checkbox" checked={dateWiseTotal} onChange={e => setDateWiseTotal(e.target.checked)} style={{ width: 16, height: 16, accentColor: '#2563eb' }} />
                    Date Wise Total
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: '#334155', cursor: 'pointer' }}>
                    <input type="checkbox" checked={openingStock} onChange={e => setOpeningStock(e.target.checked)} style={{ width: 16, height: 16, accentColor: '#2563eb' }} />
                    Opening Stock
                  </label>
                </div>
              </div>
          ...
          <div>
            ...
            <div style={{ backgroundColor: '#ffffff', borderRadius: 8, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#0f172a' }}>Item ledger : Value</h3>
                ...
                <table style={{ width: '100%', minWidth: 1000, borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f1f5f9', borderBottom: '1px solid #cbd5e1' }}>
                      ...
                      <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700 }}>Inward</th>
                      <th style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700 }}>Outward</th>
                      <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700 }}>Balance</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr><td colSpan={6} style={{ padding: 32, textAlign: 'center', color: '#64748b', fontStyle: 'italic' }}>Related Item ledger data will be displayed here.</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
