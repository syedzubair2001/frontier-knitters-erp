...
const INVOICE_DETAIL_CUSTOMER_LIST = ['-- All --', ...CUSTOMER_LIST.filter((c) => c !== '--Select--')];
...
export default function MisqueryLogisticsInvoiceDetail() {
  ...
  const handleSubmit = (e) => {
    ...
    const filtered = INITIAL_INVOICE_DETAIL.filter((r) => {
        ...
        {!submitted && (
          <form onSubmit={handleSubmit} style={{ backgroundColor: '#ffffff', borderRadius: 8, padding: '20px 24px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '16px 24px', marginBottom: 20, backgroundColor: '#f1f5f9', padding: 16, borderRadius: 8 }}>
              {radioDropdown('Company', activeCompany, setActiveCompany, company, setCompany, INVOICE_DETAIL_COMPANY_LIST)}
              {radioDropdown('Customer', activeCustomer, setActiveCustomer, customer, setCustomer, INVOICE_DETAIL_CUSTOMER_LIST)}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
              <div className="field">
                ...
                <input type="text" className="inp" placeholder="e.g. ORD-2026-4501" value={orderNo} onChange={(e) => setOrderNo(e.target.value)} />
              </div>
              <div className="field">
                <label>Ref No (%)</label>
                <input type="text" className="inp" placeholder="e.g. REF-2101" value={refNo} onChange={(e) => setRefNo(e.target.value)} />
              </div>
              <div className="field">
                <label>Invoice No (%)</label>
                <input type="text" className="inp" placeholder="e.g. EXP-INV-2026-201" value={invoiceNo} onChange={(e) => setInvoiceNo(e.target.value)} />
              </div>
            </div>
{/* Row 3 — Status radios + Invoice Date range */}
